...
const LoginForm = () => {
  ...
  useEffect(() => {
    // Verificar si el token está presente, pero no redirigir al login de inmediato.
    const token = localStorage.getItem("token");
    
    // Si el token está presente y estamos en el login, redirigir a /home.
    if (token ) {
      window.location.href = "/home";
    }
    // El login es donde el usuario debería estar si no tiene un token válido.
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(data.message || "Fallo algo al iniciar sesión");
      }
    } catch (err) {
